    ...
    public class HillCipher : ICryptographicTechnique<List<int>, List<int>>
    {
        ...
        public List<int> Decrypt(List<int> cipherText, List<int> key)
        {
            ...
            if (key.Count == 4)
            {
                ...
                int d1 = ((((key[3] * det) % 26) + 26) % 26);
                int d2 = ((((-1 * key[1] * det) % 26) + 26) % 26);
                int d3 = ((((-1 * key[2] * det) % 26) + 26) % 26);
                int d4 = ((((key[0] * det) % 26) + 26) % 26);
                List<int> plain = new List<int>() { d1, d2, d3, d4 };               
                int a1=0, a2 = 0;
        ...
        {
            List<int> cipherText = new List<int>();//Declare list to store the cipher and return it down
            int sz = (int)Math.Sqrt(key.Count);//get the size of the cipher
            if (sz * sz != key.Count)
            {
                sz++;
            }//ceil the anssssssss
            List<List<int>> mat1 = new List<List<int>>();//to store 2d list 
            //Generate the first matrix (KEY MATRIX)
            int k = 0;//Counter to increase after adding any item in the list
            for (int i = 0; i < key.Count/ sz; i++)
            {
                List<int> tmp = new List<int>();//to store the row in it
                for (int j = 0; j < sz; j++)
                {
                    tmp.Add(key[k]);//adding ele in the row
                    k++;
                }
                mat1.Add(tmp);//adding row by row in the list
            }
            List<List<int>> mat2 = new List<List<int>>();
            //Generate the Matrix of plain text
            k = 0;
            ...
            for (int i = 0; i < plainText.Count / sz; i++)
            {
                ...
                for (int r = 0; r < sz; r++)
                {
                    ...
                    tmp %= 26;//if the element gratter than 26
                    if (tmp < 0)
                    {
                        tmp += 26;//if it less than 26
                    }
                    mal.Add(tmp);
                }
                for (int j = 0; j < sz; j++)
                {
                    cipherText.Add(mal[j]);//get the cipher text
                }
            }
            return cipherText;
        }
